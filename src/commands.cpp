...
std::string Commands::login(uint64_t tag, std::string& username, const std::string& password) {
...
std::string Commands::list(uint64_t tag, const std::string& reference, const std::string& mailbox) {
    std::string command = std::to_string(tag) + " LIST " + reference + SPACE + mailbox + EOL;
    return command;
}

std::string Commands::select(uint64_t tag, const std::string& mailbox) {
    std::string command = std::to_string(tag) + " SELECT " + mailbox + "\r\n";
    return command;
}

std::string Commands::fetch_header_important(uint64_t tag, const int message_id) {
    std::string command = std::to_string(tag) + " FETCH " + std::to_string(message_id) + " (BODY.PEEK[HEADER.FIELDS (Date From Message-ID Subject)])\r\n";
    return command;
}

std::string Commands::fetch_header(uint64_t tag, const int message_id) {
    std::string command = std::to_string(tag) + " FETCH " + std::to_string(message_id) + " BODY[HEADER]\r\n";
    return command;
}

std::string Commands::fetch(uint64_t tag, const int message_id) {
